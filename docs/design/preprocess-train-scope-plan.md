...
**配套 ADR**：ADR 0010（待起草）supersedes ADR 0004 + Addendum 1
（注：ADR 0009 已被 "统一日志 + 错误体系" 占用，本提案是 0010）
**审阅档案**：`tmp/preprocess-redesign-{r1,r2,r3,user-feedback,final-summary,final-summary-v2}.md`（含 3 方 × 3 轮审阅 + 用户两轮反馈）
...
### 审阅档案
- `tmp/preprocess-redesign-r1-{arch,ux,impl}.md` — 第一轮独立审阅
- `tmp/preprocess-redesign-r2-{arch,ux,impl}.md` — 第二轮交叉审阅
- `tmp/preprocess-redesign-r3-{arch,ux,impl}.md` — 第三轮用户反馈后重审
- `tmp/preprocess-redesign-user-feedback.md` — 用户第一轮反馈 ground truth
- `tmp/preprocess-redesign-final-summary.md` — v1 汇总（4 个分歧待决）
- `tmp/preprocess-redesign-final-summary-v2.md` — v2 汇总（用户二轮反馈后）
