...
def test_loose_txt_without_image_skipped(tmp_path: Path) -> None:
    """孤立 .txt（无同 stem 图片）→ 跳过「无匹配图片」，不落盘。"""
    out = uploads.accept_one("note.txt", io.BytesIO(b"tag1, tag2"), tmp_path)
    assert out.added == []
    assert len(out.skipped) == 1
    assert out.skipped[0]["name"] == "note.txt"
    assert "无匹配图片" in out.skipped[0]["reason"]
    assert not (tmp_path / "note.txt").exists()


def test_loose_txt_with_existing_image_attached(tmp_path: Path) -> None:
    """download/ 已有同 stem 图片时，loose .txt 作为 caption 落盘。"""
    (tmp_path / "photo.png").write_bytes(b"img")
    out = uploads.accept_one("photo.txt", io.BytesIO(b"1girl, solo"), tmp_path)
    assert out.added == ["photo.txt"]
    assert (tmp_path / "photo.txt").read_bytes() == b"1girl, solo"
...
def test_zip_png_plus_txt_caption_paired(tmp_path: Path) -> None:
    """kohya 约定：zip 内 png + 同名 txt → txt 作为 caption 落 download/{stem}.txt。"""
    blob = _zip_bytes(
        {
            "a.png": b"AA",
            "a.txt": b"1girl, solo",
            "sub/b.jpg": b"BB",
            "sub/b.txt": b"landscape",
            "orphan.txt": b"no image here",
        }
    )
    out = uploads.accept_one("pack.zip", io.BytesIO(blob), tmp_path)
    assert sorted(out.added) == ["a.png", "a.txt", "b.jpg", "b.txt"]
    assert (tmp_path / "a.txt").read_bytes() == b"1girl, solo"
    assert (tmp_path / "b.txt").read_bytes() == b"landscape"
    # 无匹配图片的孤立 txt 被跳过
    assert any("orphan.txt" in s["name"] for s in out.skipped)
    assert not (tmp_path / "orphan.txt").exists()


def test_zip_caption_follows_converted_png_stem(tmp_path: Path) -> None:
    """convert_to_png 把 1.jpg 改名 1.png 时，1.txt 仍挂到最终 stem。"""
    blob = _zip_bytes(
        {
            "1.jpg": _jpg_bytes(),
            "1.txt": b"masterpiece",
    ...
    out = uploads.accept_one(
        "pack.zip", io.BytesIO(blob), tmp_path, convert_to_png=True,
    )
    assert sorted(out.added) == ["1.png", "1.txt"]
    assert (tmp_path / "1.png").exists()
    assert (tmp_path / "1.txt").read_bytes() == b"masterpiece"


def test_accept_many_pairs_loose_png_and_txt_any_order(tmp_path: Path) -> None:
    """多文件上传：txt 先于 png 发送也能配对（accept_many 先落图后挂 caption）。"""
    files = [
        ("photo.txt", io.BytesIO(b"tag")),
        ("photo.png", io.BytesIO(_png_bytes())),
    ]
    out = uploads.accept_many(files, tmp_path)
    assert sorted(out.added) == ["photo.png", "photo.txt"]
    assert (tmp_path / "photo.txt").read_bytes() == b"tag"
