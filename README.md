...
>
> **Точечно перенесено из апстрима v0.21–v0.25** (полной синхронизации версии не было):
> block swap для тренировки (Krea 2 с 16 ГБ VRAM, #463 + исправление градиентов #466)
> и его подключение к Anima — **тренировка на картах 6 ГБ** (#490, #497),
...
> а также блок исправлений по памяти: упаковка pinned-памяти вместо округления до
> степени двойки (#511, ложный «CUDA out of memory» на старте), возврат кэша
> аллокатора при смене ARB-бакета (#506), уборка остатков видеопамяти после
> неудачной генерации и сброс ссылки на модель в detach (#499).
> **Не перенесено**: block swap на стороне генерации (зависит от апстримного #455),
> переработка страницы оценки из v0.22.0 (#469–#471) и выбор вычислительной
> видеокарты на многокарточных машинах (#494).
...
ModelScope, triton. По умолчанию они лежат в домашней папке на системном диске и суммарно
занимают несколько ГБ (один только колёсный кеш CUDA torch — 2–3 ГБ), причём удаление репозитория
их не забирает. Теперь всё это идёт в `<папка проекта>/.cache/` — папку можно удалять когда угодно,
максимум что-то скачается заново.

- Уже заданные вами переменные (`HF_HOME` и т.п.) не перезаписываются.
- Хотите вернуть общесистемные кеши (например, чтобы pip-кеш был общим для нескольких копий):
  `ALS_SYSTEM_CACHES=1`.
- Проверить, что всё действительно внутри: `AnimaLoraStudio.exe --check` — строка `caches`.
...
- **GPU**：NVIDIA，**16 GB+ 显存推荐**（RTX 4060Ti 16G / 4070Ti / 4080 / 5070+ / 3090 / 4090 / 5090 等）；不开 Block 交换 **8 GB 极限可跑**（部分笔记本 GPU 实测可行，需关 sample 输出 + 减小 batch / 分辨率，且训练速度明显下降）。开 **Block 交换**（Anima 换出全部 28 层）：**训练下探到 6 GB 级显卡**（1024²、sample 照开，训练步分配峰值约 1.9 GB + 常驻文本编码器，速度约慢 11%，产出的 LoRA 不受影响）。系统 GPU 占用低，VRAM 主要给训练；A 卡 / Apple Silicon 不支持
- **RAM**：16 GB+；开 Block 交换按换出层数额外**锁定**内存（Anima 全换出约 3.6 GB，Krea 2 fp8 底模全换出约 11 GB）
