...
**Python на том же SSD** (он ставится своим установщиком и по умолчанию идёт на C:):

1. Установщик с python.org → **Customize installation** → Next → **Customize install location**
   поменять на `D:\Python313` (или куда угодно на вашем диске).
2. Галочку **Add python.exe to PATH** на первом экране лучше поставить — тогда всё найдётся само.
3. Если PATH ставить не хотите — укажите путь явно:
   `AnimaLoraStudio.exe --python D:\Python313\python.exe` (или переменная `ALS_PYTHON`).
4. Совсем портативно: положите Python в папку `python\` внутри проекта — лаунчер найдёт его сам,
   и тогда проект вместе с интерпретатором это один каталог, который можно скопировать или удалить целиком.
   **Не** годится официальный *embeddable* zip — в нём вырезаны `venv` и `pip`.

На C: при этом остаётся только сам py-лаунчер (`C:\Windows\py.exe`, пара сотен КБ) и записи в реестре.
